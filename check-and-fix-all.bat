...
echo Checking MPLibraryStubs files and fixing remaining issues...



echo.

echo Checking if files exist in MPLibraryStubs folder...

if exist "C:\VdO_Progetto\MPLibraryStubs\FileLog.cs" (

    echo FileLog.cs exists

) else (

    echo FileLog.cs NOT FOUND!

)



if exist "C:\VdO_Progetto\MPLibraryStubs\AdditionalClasses.cs" (

    echo AdditionalClasses.cs exists

) else (

    echo AdditionalClasses.cs NOT FOUND!
...
echo Adding missing WriteCtor overloads to FileLog.cs...
...
if ($content -notmatch 'WriteCtor\(object obj\)') { ^

    $content = $content -replace '(public void WriteCtor\(string message\) { })', ^

    '$1`r`n    public void WriteCtor(object obj) { WriteCtor(obj?.GetType().Name ?? \"null\"); }`r`n    public void WriteCtor(Type type) { WriteCtor(type?.Name ?? \"null\"); }'; ^

    Set-Content -Path $file -Value $content -Encoding UTF8; ^

    Write-Host 'Added WriteCtor overloads'; ^
...
echo Creating comprehensive fix with explicit references...
...
echo     public void WriteCtor^(VdO2013Core.ActionTaskManager obj^) { WriteCtor^(obj?.GetType^(^).Name ?? "ActionTaskManager"^); }

echo     public void WriteCtor^(VdO2013Core.BackgroundWorkerEx obj^) { WriteCtor^(obj?.GetType^(^).Name ?? "BackgroundWorkerEx"^); }
...
echo Verifying all stub files are in place...

dir C:\VdO_Progetto\MPLibraryStubs\*.cs
...
echo 1. In Visual Studio, right-click MPLibraryStubs in Solution Explorer

echo 2. Select "Scarica Progetto" (Unload Project)

echo 3. Right-click again and select "Ricarica Progetto" (Reload Project)

echo 4. Right-click and select "Ricompila" (Rebuild)

echo 5. Then rebuild VdO2013Core

echo.

echo If classes are still not found, check that:

echo - MPLibraryStubs is referenced by VdO2013Core

echo - All .cs files are included in the MPLibraryStubs project
